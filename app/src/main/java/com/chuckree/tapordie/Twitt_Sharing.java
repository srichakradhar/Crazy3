...
public class Twitt_Sharing {
	...
	class PostTwittTask extends AsyncTask<String, Void, String> {
		...
		protected String doInBackground(String... twitt) {
			try {
				...
				return Share_Pic_Text_Twitter(image_path, twitt_msg, mTwitter.twitterObj);
	...
	public String Share_Pic_Text_Twitter(File image_path, String message, Twitter twitter)
			throws Exception {
		String Status = "";
		try {

			StatusUpdate st = new StatusUpdate(message);
            Log.d(this.getClass().toString(), image_path.toString());
            //UploadedMedia media = twitter.uploadMedia(image_path);
